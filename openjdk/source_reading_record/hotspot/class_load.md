...
- 处理常量池`ClassFileParser::parse_constant_pool`
  - 不断获取常量池内容，根据tag类型（**17种**，没有编号2、13、14），初始化`ConstantPool`对象的tags和具体内容，一般都是一些index和基本数据类型的值。特殊: utf8的数据`JVM_CONSTANT_Utf8`要存到符号表`SymbolTable`中，所以utf8条目存的是一个符号指针`Symbol*`。接下来`JVM_CONSTANT_String`也要指向对应的`Symbol*`。（后面执行`ldc`字节码来获取一个字符串常量时，会从符号表`SymbolTable`中获取字符串，并把字符串`intern`到字符串表`StringTable`）
